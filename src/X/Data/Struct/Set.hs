...
bin x l r
  = Bin (size l `add` size r `add` 1) x l r
{-# INLINE bin #-}
...
valid t
  = balanced t `and` ordered t `and` validsize t
...
balanced t
  = case t of
      Tip         -> True
      Bin _ _ l r -> (size l `add` size r `le#` 1 `or` (size l `le#` delta `mul` size r `and` size r `le#` delta `mul` size l)) `and`
                     balanced l `and` balanced r

validsize :: Set a -> Bool
validsize t
  = (realsize t `eq#` Just (size t))
  where
    realsize t'
      = case t' of
          Tip          -> Just 0
