...
import Traversal.Internal.Bazaar
import Dimap
import I.Type
import O.Type
import Instances
...
[instances| Dimap (Baz Map t) where
  dimap f g (Baz afbft) = Baz (\a'fb' -> afbft (\a -> map f (a'fb' (g a))))|]


map'sell = sell @Map
[instances| Traverse_ (Baz Map t b) where
  traverse_ f (Baz bz) = map (\(Bazaar m) -> Baz m) ((\(O fg) -> fg) (bz (\x -> O (map map'sell (f x)))))|]
